    ...
    function jeDatePick(elem, options){
        var config = {
            language:{
                ...
                timetxt: ["time picker","start time","end time"],
                backtxt:"返回日期",
                clear  : "Clear",
                today  : "Now",
                yes    : "Apply"
    ...
    jet.extend(jeDatePick.prototype,{
        ...
        setDatas : function(){
            ...
            that.$data = jet.extend({year:false,month:false,day:true,time:false,timebtn:false},{
                shortcut:[],language:opts.language,yaerlist:[],monthlist:[[],[]],ymlist:[[],[]], daylist:[[],[]],
        ...
        dateTemplate : function() {
            var that = this, opts = that.$opts, multi = opts.multiPane,YMDStr = "",hmsStr = "",language = opts.language,
                ytxt = language.name == "cn" ? "年":"", mtxt = language.name == "cn" ? "月":"";
            ...
            '{% for(var w=0;w<language.weeks.length;w++){ %} <th>{%=language.weeks[w]%}</th> {% } %}</tr></thead><tbody>'+
            ...
            //循环时间模板
            var hmsHtml = '<div class="jedate-time">{% for(var h=0;h<timelist.length;h++){ %}<div class="timepane"><div class="timeheader">{%= timelist.length == 1 ? language.timetxt[0]:language.timetxt[h+1]%}</div><div class="timecontent">'+
            '<div class="hmstitle"><p>{%=language.times[0]%}</p><p>{%=language.times[1]%}</p><p>{%=language.times[2]%}</p></div>'+
            ...
            var paneHtml = '{% for(var i=0;i<pane;i++){ %}<div class="jedate-pane">'+
                ...
                '</div>{% } %}';   
            var btnStr = '{% if(timebtn){%}<div class="timecon" style="cursor: pointer;" @on="timeBtn">{%=language.timetxt[0]%}</div>{% } %}<div class="btnscon">{% if(clear){ %}<span class="clear" @on="clearBtn">{%=language.clear%}</span>{% } %}{% if(today){ %}<span class="today" @on="nowBtn">{%=language.today%}</span>{% } %}{% if(yes){ %}<span class="setok" @on="sureBtn">{%=language.yes%}</span>{% } %}</div>';
        ...
        methodEventBind : function() {
            ...
            jet.extend(that,{
                ...
                sureBtn:function () {
                    ...
                    opts.donefun && opts.donefun.call(that,{elem:that.valCell,val:dateVal,date:newArr}); 
                    that.valCell.focus();//用于触发表单校验
					that.valCell.blur();//用于触发表单校验
                },
                blankArea:function () {
        ...
        eachDays  : function (yd,md,ds,idx) {
            var that = this, count = 0, daysArr = [],opts = that.$opts, multiPane = jet.isBool(opts.multiPane),
                ...
                language = opts.language, endval = opts.valiDate||[],
            ...
            var isfestival = function(y, m ,d) {
                var festivalStr = '';
                if(opts.festival == true && language.name == "cn"){
